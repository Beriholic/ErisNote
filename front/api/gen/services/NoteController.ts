...
export class NoteController {
  ...
  readonly deleteNote: (
    options: NoteControllerOptions["deleteNote"]
  ) => Promise<ErisResult<Void>> = async (options) => {
  ...
  readonly getNoteDetail: (
    options: NoteControllerOptions["getNoteDetail"]
  ) => Promise<ErisResult<NoteDto["NoteController/NOTE_DETAIL"]>> = async (
    options
  ) => {
  ...
  readonly newNote: (
    options: NoteControllerOptions["newNote"]
  ) => Promise<ErisResult<Void>> = async (options) => {
    let _uri = "/note/new";
    return (await this.executor({
      ...
      method: "POST",
      body: options.body,
    })) as Promise<ErisResult<Void>>;
  };

  readonly updateNote: (
    options: NoteControllerOptions["updateNote"]
  ) => Promise<ErisResult<Void>> = async (options) => {
